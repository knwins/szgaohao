...
%>
	...
	set rs=server.createobject("adodb.recordset") 
	if request("type")<>"" then
	sql="select * from news where type="&session("type")&" and ver='cn'"
	rs.open sql,conn,1,1
	else
	sql="select * from news where ver='cn' order by orders desc"
	rs.open sql,conn,1,1
	end if
...
%>
  ...
  <TR>
    <TD width=555 valign="top"><table cellspacing="0" cellpadding="3" width="100%" border="0">
        <tbody>
          <tr>
