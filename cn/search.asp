                            ...
                            <td><table cellspacing="2" cellpadding="0" width="100%" border="0">
                                <tbody>
                                  <tr>
                                    ...
                                    <td valign="bottom" align="left" width="566"><span class="font">当前位置</span>: 搜索关键字:<%=session("key")%></td>
                                  </tr>
                                </tbody>
                            </table></td>
                          </tr>
...
%>
		...
		%>
						  ...
						  <table border="0" cellspacing="2" cellpadding="5">
                              <tr>
                                <td><a href="productshow.asp?productid=<%=rs("product_id")%>" > <img src="/<%=rs("S_IMAGE")%>" width=150 height="150" border=0></a> </td>
                              </tr>
                              <tr>
                  ...
                  <tr>
                    <td height="32" align="center"><table border="0" cellspacing="0" cellpadding="0" class="font">
                        <form name="form2" method="post" action="search.asp?key=<%=session("key")%>">
                          <tr>
                            <td align="center" height="22"> 共计<b>:<%=total%></b>&nbsp;&nbsp;<%=page%>/<b><%=rs.pagecount%></b> 页 <a href="search.asp?key=<%=session("key")%>&page=1">首页</a>
                              <%if page>1 then%>
                              <a href=search.asp?key=<%=session("key")%>&page=<%=page-1%>>上一页</a>
                              <%else%>
                              下一页
                              <%end if%>
                              <%if page<rs.pagecount   then%>
                              <a href=search.asp?key=<%=session("key")%>&page=<%=page+1%>>下一页</a>
                              <%else%>
                              下一页
                              <%end if%>
                              <a href=search.asp?key=<%=session("key")%>&page=<%=rs.pagecount%>>最后页</a>
                              <select name="page">
